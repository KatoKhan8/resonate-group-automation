...
class TestObservedIsEvidenceNotAGuess(unittest.TestCase):
    """OBSERVED was added 2026-09-18 and it is the classification this estate
    actually has evidence for. These assertions are about what it must NOT
    become: a second name for UNKNOWN, or a second name for CONFIRMED."""

    def test_an_observed_limit_is_enforced_at_its_measured_rate(self):
        """Not floored. The whole point is that the floor was wrong here.

        ContactOut people-count is 4,822 of the 8,760 provider calls in a
        5,000-record pass. At the UNKNOWN floor of 5/minute those calls alone
        take 16 hours; at the measured 1,038/minute they take minutes.
        """
        effective = ratelimit.effective_limit(ratelimit.CONTACTOUT_PEOPLE_COUNT)
        self.assertEqual(effective.rate, 1038)
        self.assertIs(effective.classification,
                      ratelimit.LimitClassification.OBSERVED)
        self.assertNotEqual(effective.rate, ratelimit.DEFAULT_LIMIT.rate)

    def test_unknown_is_still_floored_and_observed_did_not_widen_it(self):
        """The floor must still catch everything it caught before."""
        for declared in (None,
                         ratelimit.ProviderLimit(rate=9999, unit="minute"),
                         ratelimit.ProviderLimit(
                             rate=9999, unit="minute",
                             classification=ratelimit.LimitClassification.UNKNOWN)):
            with self.subTest(declared=declared):
                self.assertEqual(
                    ratelimit.effective_limit(declared).rate,
                    ratelimit.DEFAULT_LIMIT.rate)

    def test_an_observed_limit_carries_the_evidence_that_made_it_one(self):
        """A rate with no provenance is a guess with a better label.

        `source` must name where the number came from, and the date, so a
        reader can tell whether it is still true.
        """
        source = ratelimit.CONTACTOUT_PEOPLE_COUNT.source
        self.assertTrue(source, "an OBSERVED limit with no source is a guess")
        self.assertIn("2026-09-18", source)
        self.assertIn("429", source)

    def test_observed_is_not_confirmed(self):
        """An unpublished limit can change without notice. OBSERVED must stay
        distinguishable from the vendor's own word."""
        self.assertIsNot(ratelimit.LimitClassification.OBSERVED,
                         ratelimit.LimitClassification.CONFIRMED)
        self.assertIsNot(ratelimit.CONTACTOUT_PEOPLE_COUNT.classification,
                         ratelimit.LimitClassification.CONFIRMED)

    def test_the_marketing_number_is_still_not_confirmed(self):
        """EmailBison's 3,000 rpm is a features page. Unchanged by this."""
        self.assertIs(ratelimit.EMAILBISON_PUBLISHED.classification,
                      ratelimit.LimitClassification.MARKETING_PAGE)
