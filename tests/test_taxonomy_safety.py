...
class GenuineInterestStillClassifies(unittest.TestCase):
    ...
    def test_a_stated_constraint_reaches_objection(self):
        # "no budget" is deliberately absent here: TASK-066 claimed that
        # phrase for the production NEGATIVE patterns, and production rules
        # run first. See test_a_production_negative_beats_objection below,
        # which pins that precedence rather than leaving it to chance.
        for text in (
    ...
    def test_a_production_negative_beats_objection(self):
        """A stated constraint a production pattern already owns stays NEGATIVE.

        TASK-066 added "no budget|funding" to NEGATIVE_PATTERNS. The taxonomy
        only fires when the production rules return nothing, so these reach
        NEGATIVE rather than OBJECTION - and that is the SAFER of the two.
        NEGATIVE stops outreach; OBJECTION maps to UNKNOWN, which only pauses
        it. If this ever flips to OBJECTION, the system got less careful.
        """
        for text in (
            ...
            verdict = replies.classify(text)
            self.assertEqual(
                verdict["classification"], replies.NEGATIVE, text)
