...
class AReferralNeedsAHumanWriterAndAHumanTarget(unittest.TestCase):
    ...
    def test_draft_seven_is_also_refused_on_its_target(self):
        ...
        self.assertIn("buero@erlebnis.example.test",
                      decision.get("account_context") or [])

    def test_a_generic_target_is_account_context_not_an_enrolment(self):
        decision = replyengine.decide(
            {"text": "please send it to info@acme.example.test", "automated": False},
            self.as_referral(), tz_offset_hours=2)
        self.assertEqual(decision.get("account_context"), ["info@acme.example.test"])
    ...
    def test_a_named_person_passes_both_gates(self):
        ...
        decision = replyengine.decide(
            {"text": "speak to marko.juric@acme.example.test, he owns this",
    ...
    def test_the_generic_list_covers_the_shapes_we_see(self):
        for address in ("info@a.example.test", "office@a.example.test", "buero@a.example.test",
                        "kontakt@a.example.test", "hello@a.example.test", "sales@a.example.test",
                        "sales-eu@a.example.test", "office_2@a.example.test", "noreply@a.example.test"):
            with self.subTest(address=address):
                self.assertTrue(replies.is_generic_mailbox(address))

    def test_a_person_is_not_a_generic_mailbox(self):
        for address in ("marko.juric@acme.example.test", "m.horvat@studio.example.test",
                        "megan@studionorth.example.test"):
            with self.subTest(address=address):
