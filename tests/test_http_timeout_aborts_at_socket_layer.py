...
class HttpTimeoutAbortsTheSocket(unittest.TestCase):
    ...
    def test_http_timeout_IS_a_provider_error(self):
        """INVERTED, not deleted. This asserted the opposite and was wrong.

        The original reasoning was sound as far as it went: HttpTimeout is a
        timeout rather than a refusal, and a retry policy must tell them
        apart. The conclusion drawn from it - therefore it must not inherit
        ProviderError - does not follow, and it was a regression.

        Before this classification existed a timeout arrived as a plain
        `ProviderError`, and **27 `except ProviderError` sites across `src/`**
        catch exactly that - in `enrich`, `bisonfactory`, `campaigns` and
        elsewhere. Measured: with `HttpTimeout(TimeoutError)` alone,
        `raise HttpTimeout(...)` escaped `except ProviderError` entirely. A
        timeout would have stopped being "this provider call failed, record it
        and carry on with the other records" and become an uncaught exception
        that ends the pass - on the failure a long provider-bound run is most
        likely to hit.

        Distinguishability comes from the TYPE, never from the absence of a
        base class: `except HttpTimeout` still selects it precisely, and
        `test_http_timeout_is_a_timeout_error` above still holds.
        """
        self.assertTrue(issubclass(HttpTimeout, providers.ProviderError))
...
class TestATimeoutIsStillAProviderError(unittest.TestCase):
    """The classification must not make a timeout invisible.

    `HttpTimeout` was first written as `HttpTimeout(TimeoutError)` alone, so
    that "inheriting from nothing else" would keep it distinguishable from a
    transport error, a refusal and a 5xx. It was distinguishable and it was
    invisible: before the change a timeout arrived as a plain `ProviderError`,
    and 27 `except ProviderError` sites across `src/` catch exactly that.

    A timeout that escapes them turns "this provider call failed, record it
    and carry on with the other records" into an uncaught exception that ends
    the pass - on the failure a long provider-bound run is most likely to hit.
    """

    def test_a_timeout_is_caught_by_except_provider_error(self):
        from src.providers import HttpTimeout, ProviderError
        self.assertTrue(issubclass(HttpTimeout, ProviderError))
        try:
            raise HttpTimeout("simulated")
        except ProviderError:
            pass
        except Exception as exc:                          # pragma: no cover
            self.fail(f"a timeout escaped `except ProviderError` as "
                      f"{type(exc).__name__}; 27 call sites would crash")

    def test_it_is_still_selectable_as_a_timeout(self):
        """Distinguishability comes from the type, never from the absence of
        a base class."""
        ...
        try:
            ...
            self.fail(f"not recognised as a TimeoutError: "
                      f"{type(exc).__name__}")

    def test_a_transport_error_is_not_a_timeout(self):
        """A refused connection demonstrably did not arrive; a timeout may
        have. A retry policy reads that difference, so the two must not
        collapse."""
        from src.providers import HttpTimeout, HttpTransportError
        self.assertFalse(issubclass(HttpTransportError, HttpTimeout))
        self.assertFalse(issubclass(HttpTimeout, HttpTransportError))

    def test_every_provider_error_handler_in_src_still_sees_a_timeout(self):
        """The count is the point: this is not one call site, it is 27."""
        import pathlib
        import re
        root = pathlib.Path(__file__).resolve().parent.parent / "src"
        hits = 0
        for path in root.rglob("*.py"):
            hits += len(re.findall(r"except ProviderError",
                                   path.read_text(encoding="utf-8")))
        self.assertGreater(hits, 20,
                           "if this collapsed, the reason to inherit "
                           "ProviderError needs re-checking rather than the "
                           "test loosening")
