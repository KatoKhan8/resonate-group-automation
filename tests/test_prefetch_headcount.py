...
from src import enrich, gather, gather as gather_mod, store, waterfall
...
class TestTheBridgeRefusesAnythingButTheFreeCall(unittest.TestCase):
    """The prefetch spend bridge is scoped to `people-count` by name.

    The real `spend` inside `enrich_record` enforces "company first" - no paid
    person-level call before an explicit ICP verdict - and TASK-205's evidence
    gate. The prefetch runs one level up, in `run`, so it cannot reuse that
    closure and needs an adapter. For a free company-level call the two are
    equivalent; for a paid person-level call they are not, and the adapter
    enforces neither gate.

    A reader adding a second call to the prefetch would have no reason to
    notice that. So the bridge refuses by name.
    """

    def test_a_paid_person_level_call_through_the_bridge_raises(self):
        import src.enrich as enrich_mod
        source = inspect.getsource(enrich_mod.run)
        self.assertIn("the prefetch spend bridge is scoped to 'people-count'",
                      source,
                      "the bridge must refuse a call it does not gate")

    def test_the_refusal_names_the_gates_it_does_not_enforce(self):
        """A refusal that does not say WHY sends the reader to work around it."""
        import src.enrich as enrich_mod
        source = inspect.getsource(enrich_mod.run)
        for owed in ("company-first", "evidence gate", "enrich_record"):
            with self.subTest(mentions=owed):
                self.assertIn(owed, source)


class TestTheProviderCallIsLoggedOnTheRecord(unittest.TestCase):
    """A provider call that leaves no trace on the record it was made about.

    `tests/test_enrich.py::test_every_provider_call_is_logged_on_the_record`
    failed when the prefetch first landed, because the prefetched branch wrote
    `headcount_signal` and the serial branch's `store.log` line then never
    ran. Moving a call does not change what the record owes a reader.
    """

    def test_on_applied_runs_inside_the_serial_apply_loop_in_input_order(self):
        seen = []
        records = [{"id": f"r{i}", "domain": f"d{i}.test", "company_facts": {}}
                   for i in range(12)]

        def people_count(domain):
            return {"profiles": int(domain[1:].split(".")[0]) + 100}

        report = gather.prefetch_headcount(
            records, people_count, lambda *a, **k: True, k=8,
            on_applied=lambda rec, count: seen.append(
                (rec["id"], count["profiles"])))

        self.assertEqual(report["ok"], 12)
        self.assertEqual(seen,
                         [(f"r{i}", i + 100) for i in range(12)],
                         "on_applied must run in INPUT order, not completion "
                         "order - the log lines land where a serial pass "
                         "would have written them")

    def test_a_failed_call_does_not_log_a_success(self):
        seen = []
        records = [{"id": "ok", "domain": "ok.test", "company_facts": {}},
                   {"id": "bad", "domain": "bad.test", "company_facts": {}}]
        ...
        report = gather.prefetch_headcount(
            records, people_count, lambda *a, **k: True, k=4,
            on_applied=lambda rec, count: seen.append(rec["id"]))
        self.assertEqual(seen, ["ok"])
        self.assertEqual(report["failed"], 1)

    def test_a_budget_refusal_does_not_log_a_call_that_did_not_happen(self):
        seen = []
        records = [{"id": "r1", "domain": "d1.test", "company_facts": {}}]
        report = gather.prefetch_headcount(
            records, lambda domain: {"profiles": 3},
            lambda *a, **k: False, k=2,
            on_applied=lambda rec, count: seen.append(rec["id"]))
        self.assertEqual(seen, [])
        self.assertEqual(report["skipped_budget_refused"], 1)
