...
def audit():
    ...
    for dotted in sorted(info):
        ...
        for consumer_dotted, imported_names in sorted(raw_consumers.items()):
            ...
            # Module import with alias: imported name is used as a base in dotted access
            # e.g., ``from . import clientapproval as ca`` then ``ca.APPROVED``
            for local_name in imported_names:
                if local_name in pub:
                    continue  # Already handled above
                # Check if this local name is used as a base in dotted accesses
