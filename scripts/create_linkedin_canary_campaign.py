#!/usr/bin/env python3
"""Create the LinkedIn DRAFT canary campaign bound to list 940797. Authorized.

    py -3 scripts/create_linkedin_canary_campaign.py            # dry run
    py -3 scripts/create_linkedin_canary_campaign.py --live      # create it

AUTHORIZED 2026-09-16: "APPROVED - enable heyreach.create_campaign for list
940797". Recorded in OPERATOR-AUTHORIZATION-2026-09-16.md.

MAXIMUM SEND EXPOSURE OF THIS SCRIPT IS ZERO. `heyreach.create_campaign`
creates in DRAFT, and its own docstring records why that is the safety
property: "A DRAFT SENDS NOTHING. Activation is /campaign/StartCampaign, which
is not on WRITE_ROUTES and is not implemented here." `heyreach.activate` is NOT
in SUPPORTED and a test asserts it.

THE BIND IS THE POINT, AND IT IS ONE-WAY. TASK-216 established that
`linkedInUserListId` is supplied at creation and there is no separate attach
route. So the moment this succeeds, list 940797 stops being unbound - the
staging safety property it has carried all day ends, and
`liststaging.assert_list_safe` will correctly refuse every later add to it.
A future cohort needs a different unbound list. That is expected, not a bug.

WHAT IT REFUSES BEFORE WRITING. The `CONDITIONAL` predicate
`_list_is_ours_and_unbound_and_holds_approved` reads list 940797 from the
provider at the moment of the write and refuses unless it is ours, in our
tenant, unbound, and holding only approved leads. This script additionally
refuses if the list does not hold exactly the one approved canary.
"""
import argparse
import hashlib
import json
...
LIST_ID = 940797
SEAT_ID = 174892
NAME = "RESONATE - PRODUCTIVE LINKEDIN CANARY - CONTROL"
EXPECT_LEADS = 1
