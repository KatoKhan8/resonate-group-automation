        ...
        for contact in record.get('contacts', []):
            contact_data = {
                **record_context,
                ...
                # Specialties is a COMPANY fact, not a contact field. An
                # earlier version read contact.get('specialties') here,
                # found nothing, and reported 'specialties is 0%' as a
                # correction to a brief that said 73%. It is 74%, in
                # company_facts - the same wrong-lookup trap that
                # reported industry and headcount at 0% three times by
                # reading `sizing` instead.
                'specialties': get_field(record, 'company_facts.specialties') or [],
