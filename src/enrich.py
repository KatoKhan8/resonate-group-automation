...
def run(live=False, cap=None, limit=None, ids=None, states=("queued", "enriched"),
    ...
    if live and targets and prefetch:
        ...
        def _spend(call, why, provider="contactout", rec=None, **_kw):
            # THIS BRIDGE IS FOR ONE FREE CALL AND REFUSES TO BE ANYTHING
            # ELSE.
            #
            # The real `spend` is a closure inside `enrich_record` - it
            # captures that record, its log and its budget - so the prefetch,
            # which runs one level up in `run`, cannot reuse it and needs an
            # adapter. For `people-count` the two are equivalent: cost
            # lookup, budget charge, waterfall step.
            #
            # They stop being equivalent the moment anybody points this at a
            # PAID PERSON-LEVEL call, because the real `spend` also enforces
            # "company first" - no paid person-level call before an explicit
            # ICP verdict - and TASK-205's evidence gate. This adapter
            # enforces neither, and a reader adding a second call here would
            # have no reason to notice. So it refuses by name rather than
            # trusting the next reader.
            if call != "people-count":
                raise RuntimeError(
                    f"the prefetch spend bridge is scoped to 'people-count' "
                    f"and was asked for {call!r}. It does not enforce the "
                    f"company-first gate or the evidence gate that "
                    f"enrich_record's own `spend` applies to person-level "
                    f"calls, so routing one through here would skip both. "
                    f"Wire the paid route through `enrich_record`, and read "
                    f"gather's timeout note first: a timed-out call still "
                    f"reached the provider and still cost a credit.")
            cost = COSTS.get(call, 0)
            if not budget.charge(cost, f"{rec['id']}:{call}", call=call):
        ...
        # THE SAME LINE THE SERIAL PATH WRITES, in the same words, from
        # inside the serial apply loop and in input order. Without it a
        # provider call stops appearing on the record it was made about,
        # which `test_every_provider_call_is_logged_on_the_record` asserts
        # must never happen - and which it correctly failed on when the
        # prefetch first landed.
        def _logged(rec, count):
            store.log(rec, "enrich",
                      f"people-count: {count.get('profiles')} profiles "
                      f"(free, prefetched)")
