...
def sending_schedule(campaign_id, day):
    ...
    #
    # A MISSING OR UNREADABLE COUNT RAISES. It used to fall back to 0, which
    # is the one value that must never be guessed here: this feeds a monitor
    # whose whole job is to notice that nothing is going out, and a silent
    # zero on a shape we did not understand reports a healthy campaign as
    # silent. Same argument the module comment above makes about the 400, and
    # the same one `_collection` makes on the HeyReach side - a body of the
    # wrong SHAPE is a contract violation, not a result.
    #
    # A provider-stated 0 is a real zero and passes through untouched. The
    # distinction being kept is between "the provider said none" and "we
    # could not tell".
    if not isinstance(body, dict) or "emails_being_sent" not in body:
        raise ProviderError(
            f"emailbison sending_schedule: no `emails_being_sent` in the "
            f"response for campaign {campaign_id} on {day}. Refusing to read "
            f"that as zero - this answer feeds a monitor that watches for "
            f"nothing going out")
    raw = body.get("emails_being_sent")
    try:
        count = int(raw)
    except (TypeError, ValueError):
        raise ProviderError(
            f"emailbison sending_schedule: `emails_being_sent` is "
            f"{type(raw).__name__}, not a number, for campaign "
            f"{campaign_id} on {day}") from None
