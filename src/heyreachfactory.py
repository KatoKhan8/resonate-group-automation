...
def _refuse_qa(plan, recs, report):
    ...
    Runs immediately after the dry-run check and before the first provider
    call. Raises `FactoryRefused` carrying the runner's own rendered table,
    not a sentence written at the raise site, so a rule that did not exist
    ...
    # NOT_IMPLEMENTED checks are advisory during the rollout.
