...
### 6.1 Sendability
```
sendable = two independent vendors have approved the same normalized
           address, and nothing has refused it
```
That is stricter than this section used to state, and the older wording -
`verdict == "valid" or (accept_all and reoon.is_safe_to_send)` - is wrong
in the unsafe direction, because it omits the confirmation count. A single
`valid` from the primary is `held`, not sendable: `verification` requires
`required_confirmations: 2` and `lint.sendable` delegates to it. An
explicit refusal from any vendor - `is_safe_to_send: false` or
`is_deliverable: false` - is a disagreement and holds, whatever the others
say. `PLAYBOOK.md` states this correctly and always did; this file did
not, which matters because `CLAUDE.md` says to read this one first.
