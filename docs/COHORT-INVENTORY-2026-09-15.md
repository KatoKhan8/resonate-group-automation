...
| specialties | 68 | 73.9% | `company_facts.specialties` - NOT a contact field |
...
**CORRECTED BY CLAUDE ON REVIEW.** This section originally read "Specialties is 0%... This is not a lookup error - the field is absent from the estate." It is 73.9%, and it WAS a lookup error.

Specialties is a COMPANY fact. It lives in `company_facts.specialties`, on 164 records. The analysis read `contact.get('specialties')`, found nothing, and reported the absence as a correction to the brief - which is the most confident possible form of the mistake.

This is the FOURTH time in two days that a reader has looked in the wrong place and reported a zero here. Industry and headcount were reported at 0% three times by reading `sizing`, which is null everywhere, instead of `company_facts`. **A zero and a wrong lookup are indistinguishable from the outside**, and the task file said so explicitly before this task began.

Specialties is therefore available as a cohort signal at 73.9% coverage, with a safe fallback required for the remaining 26%.
...
- **It does not use contested fields as primary signals.** Headcount is contested and is not a cohort key. Specialties is available at 73.9% (see the correction above) and was not used here only because this pass did not read it.
