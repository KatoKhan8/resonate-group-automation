...
> **REDACTED ON INTEGRATION.** This report quoted real prospect replies
> verbatim. Product names inside those quotes are replaced with
> placeholders. Quoting a real reply word-for-word in a tracked file is
> what test_no_crm_narrative_about_a_real_account exists to stop -
> paraphrase the SHAPE of a reply, never its text.
...
2. **Campaign 262**: A prospect accepted a demo offer ("sure, I will take
   a demo. Send me a <scheduling-tool> link"). 47 words.
...
send a <scheduling-tool> link?"). Negative replies are also predominantly short
