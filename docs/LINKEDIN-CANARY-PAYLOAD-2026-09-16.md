...
**For px-d814b9bcb642 (#1):** li1-li5 have operator-control-arm approval. The approved copy matches the CONTROL fallbacks exactly (the CONTROL was written to be the fallback). li6 is generated but has no graph slot.

**For px-ecfd847755c1 (#2) and px-b8cf25c80e28 (#3):** All steps are generated (no operator approval). The `customUserFields` would carry the generated text, NOT the CONTROL fallbacks. The fallbacks fire only when `customUserFields` is missing a variable entirely.
...
**The practical effect:** 2 of 3 canary contacts (px-ecfd847755c1, px-b8cf25c80e28) have no operator-approved copy. Their generated copy was never approved through the CONTROL arm. They cannot be staged until their cadence steps are replaced with CONTROL fallbacks or operator-approved generated copy.

Only px-d814b9bcb642 has operator-control-arm approval for li1-li5, which maps to all 8 required roles. He is the only contact of the three who can be staged today.
