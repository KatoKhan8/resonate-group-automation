...
## CLAUDE REVIEW - ACCEPTED, AND ONE CORRECTION VERIFIED BY HAND, 2026-09-15

The recommendation is ONE SEAT for the first campaign and the arithmetic holds:
one seat at 40 connection requests a day clears 50 leads in 1.25 working days
and 122 in three, while the cadence's own 1-3 day delays dominate the elapsed
time either way. A second sender does not make the first campaign faster. It
adds load to a seat already carrying the client's own IN_PROGRESS campaigns,
and all 33 healthy seats are in that position - zero are uncommitted.

The task said "one seat is enough" was a legitimate answer. It is the answer.

### THE CORRECTION, CHECKED RATHER THAN BELIEVED

The worker reports that `LINKEDIN_ASSIGN_SENDER`'s OPERATIONS entry - "no
documented route ... assignment was done by hand" - is stale. Verified
directly rather than taken on the report:

    heyreach.WRITE_ROUTES     /campaign/AddLinkedInAccountsToCampaign
                              /campaign/RemoveLinkedInAccountsFromCampaign
    implementations           heyreach.add_senders    (line 1813)
                              heyreach.remove_senders (line 1831)

Both present. The entry was wrong.

It is corrected on master rather than deleted, with the reason written into it.
This repository has now been bitten three times by a paragraph about the
environment believed without checking: this module's own docstring said
SUPPORTED was empty while two sequence-write routes were live; QWEN.md told a
worker it had no credentials while it held all three; and this entry said a
route did not exist while two did. The pattern is the finding.

`LINKEDIN_ASSIGN_SENDER` stays OUT of SUPPORTED - but the reason changed, and
the entry now says so. It is no longer "no route". It is "nothing needs it",
which is a claim that expires the moment a cohort is larger than one seat's
ceiling.
