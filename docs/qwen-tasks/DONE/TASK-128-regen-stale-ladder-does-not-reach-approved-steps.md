...
## CLAUDE REVIEW - ACCEPTED, AND IT CORRECTED MY HYPOTHESIS

I wrote the task believing approval was the blocker, because three of the four
failing steps were approved. **It was not approval. It was gate ordering.**

The ladder staleness check sat at the END of the gate chain, after lint,
claims, foreign_product and quality. A step failing any earlier gate hits
`continue` and never reaches the staleness check. All four failing contacts
failed lint on `em_dash`, so their staleness was never evaluated - and the
impact report said 0 because it had genuinely found 0, having never looked.

The task told the worker to find the exact line rather than confirm the
hypothesis, and that is the only reason the right answer came back.

### The reasoning for the fix is sound

Lint asks *"does this pass the current rules"*. Ladder staleness asks *"was
this generated against the current ladder"*. Independent questions, and a step
can fail both - an em dash from a ladder that allowed it is exactly that case.
Regenerating against the current ladder answers both at once, so reporting
ladder-stale is the right op even when lint has already failed.

The ordering was accidental: lint came first, TASK-083 added staleness later,
and the dependency went unnoticed.

### VERIFIED

    before the fix, per blocked record:   steps to re-plan 0, approvals 0
    after the fix, whole estate:          steps to re-plan 187, approvals 87

87 where the pre-regeneration run reported 83 - the count went UP because
steps that were previously invisible to the check are now counted. That is the
fix working: it reveals work that was hidden, rather than creating it.

    ladder propagation + generate + ladder impact   98 tests  REAL_EXIT=0
