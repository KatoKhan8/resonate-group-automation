...
## CLAUDE REVIEW - ACCEPTED, AND ITS SIDE FINDING WAS THE BIGGER ONE

It asked the dangerous question first, which is what the task required: ARE
the declared openings honoured, or would comparing them be comparing labels?
**Honoured 10 of 12, and 3 of 3 for conversational, problem_led and value_led**,
with hand-judged evidence per approach. So a semantic comparison would reflect
copy, not tags. Unique openings would go from 2 to 3-4 per step.

CAVEAT ON ITS OWN NUMBERS: measured on a FIXTURE record ("anna" / "Acme
Agency"), not on the real estate. The honour rate is a fixture measurement and
should be re-taken on a real record before the check is changed on its
strength.

### The side finding, verified and FIXED by Claude in this commit

`style_for(node_type, approach)` resolves an unrecognised key by falling back
to the EMAIL table. `generate.py:1556` passed `spec.get("channel")`, which is
`"linkedin"` - not a node type. So it did not MISS the mapping, it got the
WRONG CHANNEL'S mapping, silently:

    style_for("linkedin_message", "value_led")  ->  peer_to_peer
    style_for("linkedin",         "value_led")  ->  professional   (email)

Every LinkedIn variant built through `build_variant_set` was styled as email,
and both answers are valid style names so nothing looked wrong from outside.

Fixed at the call site using `cadence.NODE_TYPE_FOR_CHANNEL`, which already
existed and which `cadence.variant_node` already used - with the same
precedence, so the two cannot disagree. `style_for`'s fallback is deliberately
LEFT in place (other callers may rely on it) and is now pinned by a test that
documents the trap instead of hiding it.

    tests/test_a_channel_is_not_a_node_type.py   7 tests
    variant + cadence neighbours               172 tests  REAL_EXIT=0
    generate + gates                           185 tests  REAL_EXIT=0
