...
## CLAUDE REVIEW - ACCEPTED. IT GAVE THE HONEST ANSWER IT WAS INVITED TO GIVE.

**"Multi-sender is not needed for a 50-lead cohort. One sender handles it in
2 days."** The task said that if one seat could carry the cohort, saying so was
worth more than a plan nobody needs, and it said so. Multi-sender here is about
resilience and appearance, not capacity, and 2 days is already the floor for
LinkedIn cadence timing.

That settles a policy question with arithmetic rather than with preference:
the first cohort does not need multiple senders, and the multi-sender work is
not on the critical path to going live.

### THE FINDING THAT IS NOT ABOUT US, AND MATTERS ANYWAY - VERIFIED

    seat 129531        isActive: true,  authIsValid: FALSE
    campaign 523987    IN_PROGRESS,     33,710 leads

Verified independently by Claude against the provider. **A live campaign with
33,710 leads is attached to a seat that cannot authenticate.** Those leads are
queued behind a sender that cannot act, and the seat reports itself available
the whole time - which is exactly why `authIsValid: false` was called "worse
than absent" when the estate was first measured.

This is the client's own pre-Resonate campaign, not ours. It is out of scope to
change and it is not ours to pause. But it is costing them now, it was found by
reading their estate honestly, and the operator should know.

### WHAT REMAINS TRUE AND CONSTRAINING

48 campaigns already use multiple senders, so the mechanism is proven at the
provider. `LINKEDIN_ASSIGN_SENDER` is still NOT in `providerwrites.SUPPORTED`
and this task correctly did not touch it. And zero healthy seats are
uncommitted, so every plan to add a sender is a plan to redirect a seat already
serving 8-12 of the client's campaigns - which could push it into cooldown.
