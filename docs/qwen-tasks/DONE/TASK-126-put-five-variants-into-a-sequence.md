...
## CLAUDE REVIEW - ACCEPTED, WITH THE DUPLICATED EXTRACTOR REMOVED

The wiring is right: variants are COLLECTED rather than resolved, order is
preserved so arm identity is positional, the factory refuses to pad four
variants into five, and `validate_sequence_for_write` already accepted
multi-message nodes without change.

### THE RISK IT NAMED IN ITS OWN REPORT WAS REAL

It wrote: *"Text extraction is duplicated ... this is necessary because
`_step_copy` requires step-level approval, which variants don't have."*

The obstacle is real; duplicating the extractor was the wrong answer to it,
and **the copy had already drifted from the original in the same commit that
created it**:

    DROPPED    `_step_copy`'s `effective_channel != "linkedin"` check, so a
               non-LinkedIn variant could reach a LinkedIn payload
    REVERSED   InMail field precedence, `message or note` becoming
               `note or message` - a variant carrying both sends different
               words depending on which path reads it

Neither is visible from outside. Both produce plausible text.

Fixed by lifting the variant's OWN approval - already verified by fingerprint
on the line above - onto the stepped copy, then calling the one extractor.
The obstacle is cleared honestly rather than routed around, and there is one
set of guards again.

`tests/test_a_variant_goes_through_the_same_extractor.py` pins both, on
returned values.

### THE FIVE-ARM DEMO IS SYNTHETIC, AND THE REPORT SAYS SO

Finding 5 is the honest one: the real record produces **ZERO arms**, because
no step on it has approved copy, and the factory correctly refuses unapproved
copy. The five-arm payload came from a synthetic record. TASK-037 was salvaged
and its numbers rejected for measuring a synthetic estate; this capability
claim rests on the same kind of evidence and should be re-demonstrated on a
real record once regeneration and approval have caught up.

**What is proven: the factory can build a multi-variant node. What is not:
that any real record currently has five approved variants to put in one.**

    heyreach factory + ensure_leads + seals + variants + gates
      327 tests  REAL_EXIT=0
