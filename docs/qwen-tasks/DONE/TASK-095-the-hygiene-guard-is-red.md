...
## CLAUDE REVIEW - ACCEPTED. THE GUARD IS GREEN FOR THE FIRST TIME.

Four workers ran this; their fixes were compared and ONE was taken
(`qwen-worker-2-r6`). Three of the four were byte-identical.

TWO STALE TESTS - fix verified narrower, not weaker:

    PHONE  \+\d[\d\s().-]{7,}\d   ->   \+\d[\d ().-]{7,}\d

`\s` includes the newline, so a markdown table of numbers spanning two lines
matched as a phone number. Proven: four real international formats are still
caught, the cross-newline false positive is gone. The fixture email moved to
an RFC-reserved `.example` domain.

THE REAL LEAK - 16 tokens across 9 files, all redacted to stable pseudonyms:

    scripts/task065_run_bcd.py                    6
    docs/HEYREACH-PROVIDER-READBACK-2026-09-14    2
    docs/CONTEXT-RESET-2026-09-14-C               2
    five further docs                             1 each

`scripts/task065_run_bcd.py` is the file checkpoint D said had been redacted
and that "the script now reads its record ids from the queue". It still
carried six. **A claim that a leak was fixed is not a fix**, and this one had
been believed for a day.

One of the sixteen was mine, added hours earlier in the TASK-089 task file.

MITIGATION, NOT ERASURE. Every one of these remains in git history. A history
rewrite is an operator decision and has not been taken.

    tests.test_fixture_hygiene        11 tests  REAL_EXIT=0   <- was red
    secrets + task062 + invariants   104 tests  REAL_EXIT=0
