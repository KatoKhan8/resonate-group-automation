...
## CLAUDE REVIEW - ACCEPTED. THIRD NO, AND NOW THE PATTERN IS THE FINDING.

The four TASK-130 defects are GENUINELY GONE - 0 of 12 on all four, confirming
the wider re-measure. And the verdict is still DOES NOT BEAT FALLBACKS, zero of
12 sequences sendable.

**Three human reads, three NOs, and each fix has traded a named defect for a
new form of sameness:**

    ban "just checking in"        -> the monotony moved to the next phrase
    require an easy out           -> 10 of 12 closes open identically
    remove the repeated greeting  -> every sequence walks the same six-beat arc

The read names the third one precisely: *"All 12 LinkedIn sequences follow the
same six-beat arc: intro -> question -> problem -> product -> another angle ->
templated close. The fallbacks avoid this because each has a different shape."*

**That is the structural answer after three rounds.** Variants vary the WORDS
INSIDE a step. Nothing varies the SHAPE OF THE SEQUENCE, because the ladder is
one ladder and every contact walks it identically. More phrase-level fixing
cannot reach a bar that is set by shape.

### The fallbacks, read as data rather than as a benchmark

    connection_note   90 chars   who I am, and why connect
    connected_1       99 chars   a question about visibility
    connected_2      115 chars   a peer observation, then "is that how it works
                                 for you?"
    connected_3       94 chars   what the product joins up, then "worth a look?"
    connected_4       83 chars   graceful exit AND a referral ask

Four steps, 83-115 characters each, lower case, no claim about the recipient,
and **four genuinely different shapes**. The generated copy is six steps of
roughly double the length walking one shape.

They are the best copy in the system by three independent reads, they are
already in the client config, and the operator wrote them.

### What this means for shipping

The standard is "beats the fallbacks". **The fallbacks meet it by
construction.** Shipping a cohort on the hand-written cadence is not weakening
a gate - it is using the copy that passes the gate - and it starts the outcome
loop that every later copy decision needs evidence from.

The generator keeps improving as a parallel arm, measured against live results
rather than against a fourth read. TASK-137 carries the campaign build.
