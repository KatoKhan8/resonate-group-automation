...
## CLAUDE REVIEW - ACCEPTED, AND THE FUNNEL POINTS STRAIGHT AT THE FIX

    70  economic_buyer cohort (TASK-096's largest honest cohort)
   -19  exclusion: unverified email - 12 unknown, 6 accept_all_uncleared, 1 held
   -25  quality gates: 23 banned phrase, 2 unsupported claim
   ---
    26  batch, 37% of the cohort

Zero duplicates, zero greeting failures, no structural diversity warnings.
No provider write performed. The drop-off per stage is exactly what the task
asked for and it is the most useful thing in the report.

### VERIFIED BY CLAUDE, and it changes what to do next

The dominant gate failure is ONE PHRASE IN ONE STEP, and the regeneration
already under way removes it:

    'just checking in'   li5: 26,  li6: 1   =  27 occurrences
      in REGENERATED (fingerprinted) steps:   0
      in STALE steps:                        27

**Every occurrence is in pre-fix copy and none survives regeneration.** So the
25 quality-gate losses are not a copy problem needing per-record repair - they
are the same staleness the ladder fix addresses, and the cohort recovers from
26 toward 51 as regeneration covers those records. Continue the ladder rather
than hand-fixing li5.

The 19 verification losses are a different question and do NOT recover by
regenerating: they need credits spent on the waterfall, which is a cost
