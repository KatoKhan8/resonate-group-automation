...
- `docs/LI6-AND-APPROVAL-2026-09-16.md`: full analysis document
- `scripts/task179_approval_analysis.py`: verification script

**FINDINGS:**

**li6 verdict:** The cadence named a step that should not exist. The graph was designed for 5 LinkedIn steps (li1-li5), and the cadence defined 6 (li1-li6). li6 had no graph position and never fired. The fix removes li6 from the cadence. The test `test_cadence_graph_agreement.py` fails if the cadence and graph ever disagree on step count again.

**Approval mechanism:** `approval.by` records who approved a step. `operator-control-arm` means a human operator approved the CONTROL fallback text. `claude` means the step was generated by Claude but not human-approved. Contact #1 has operator-control-arm approval for li1-li5. Contacts #2 and #3 have no operator approval.

**CONTROL-fallback canary:** The CONTROL fallbacks pass both lint and claims for all three canary contacts. Replacing generated copy with CONTROL fallbacks would make contacts 2 and 3 staging-eligible, subject to approval. This requires both a data change (replace copy in queue) and an approval change (approve steps via CLI).

**RISKS:**
- The cadence change affects all records using `PRODUCTIVE_LI_HEAVY_V1`. Records with li6 in their cadence will now have an extra step that doesn't match the cadence definition. This is harmless - the factory only looks at steps defined in the cadence.
- The test pins the current state. If somebody needs to add a 6th LinkedIn step in the future, they must also add a graph position for it.

**RECOMMENDED CLAUDE ACTION:**
- Review the cadence change and test
- Approve the CONTROL fallbacks for contacts 2 and 3 if the canary is to proceed
- Consider whether the queue needs to be updated to remove li6 from existing records (likely not necessary, as the factory ignores extra steps)
