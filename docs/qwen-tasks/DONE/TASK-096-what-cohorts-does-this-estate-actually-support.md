...
## CLAUDE REVIEW - ACCEPTED, WITH ITS HEADLINE CORRECTION REVERSED

The cohort work is good and the counts reproduce. One finding is wrong and it
was the one presented most confidently.

**"Specialties is 0% - the field does not exist in the estate. This is not a
lookup error."** It is 73.9%, on 164 records, and it WAS a lookup error. The
script read `contact.get('specialties')`; specialties is a COMPANY fact living
in `company_facts.specialties`. The task file warned about this exact trap -
industry and headcount were reported at 0% three times by reading `sizing`
instead of `company_facts` - and it happened a fourth time.

Fixed in `scripts/analyze_cohorts.py`, re-run, report corrected.

VERIFIED INDEPENDENTLY BY CLAUDE against work/queue.jsonl:

    contacts on not-dropped records   92
    economic_buyer                    70-72   (rule-dependent)
    Advertising Services              51-54
    OVERLAP                           40-42
    UNION                             81-84

**The overlap is the operational finding.** Two cohorts of 50+ exist, but they
are not two independent cohorts - their union is ~84 of 92 contacts. This
estate supports ONE cohort, or two heavily overlapping ones that would
double-count 40 people. Running both as separate campaigns would contact the
same person twice under two hypotheses and destroy the attribution.

That decision is the operator's and is recorded in TASK-101.
