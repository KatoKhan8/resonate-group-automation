...
## CLAUDE REVIEW - INTEGRATED WITH ONE GAP RECORDED, 2026-09-15

ACCEPTED. `src/research.py` and `tests/test_research_ttl.py` taken onto master.

The wiring is real and the worker proved it the way the task asked: the grep
output shows `stale_evidence` called from `why()` at line 176, not merely
defined. `age_of` computes from `retrieved_at` rather than reading the NULL
`age_days` column, which is the trap the task named and the one this
repository has fallen into before.

Neighbours run green: test_research_ttl (22), test_research_audit,
test_research_spend, test_evidence_ageing, test_our_own_words_are_not_evidence,
test_enrich, test_enrich_batch, test_a_lapsed_verdict_is_still_evidence,
test_evidence_has_one_shape.

Six of 203 records re-crawl today, all `team` rows aged 5-6 days. That is the
right shape of number: the short-lived TTL catches a stale team page without
sending the 197 whose evidence is current back to a paid crawl.

### THE GAP, AND THE WORKER NAMED IT ITSELF

Requirement 5 - a refresh REPLACES the aged rows and keeps the rest - is NOT
met. `run()` appends. The worker said so under RECOMMENDED CLAUDE ACTION
rather than claiming the requirement, which is the right way to hand over an
unfinished half.

It matters more than it looks, and for a reason neither of us had in view.
`research.for_prompt` takes `existing_evidence(rec)[:limit]` - **the first
three rows in STORED order**. Appending puts refreshed evidence at the END, so
a record that re-crawls still shows the model the stale rows it re-crawled to
replace. The TTL would fire, cost a crawl, and change nothing the prompt sees.

That is this repository's recurring defect wearing a new hat, and it is now
recorded as TASK-146 rather than fixed here, because the same function has a
second instance of it: `generate.py:526` passes unfiltered `for_prompt` output
as `public_evidence` while TASK-135's quality-filtered `research_block` is
added ALONGSIDE it as `research`. The model receives both the facts and the
furniture. One fix should answer both.

### WHAT WAS NOT TAKEN

The branch also carried tests/test_campaign_audit.py,
tests/test_nothing_talks_back_to_a_prospect.py and tests/test_providers.py.
Those are the three sibling seals master narrowed in 7eb4a73 after the full
suite found them; the branch predates that and its copies are the old ones.
Taking them would have reverted the narrowing.
