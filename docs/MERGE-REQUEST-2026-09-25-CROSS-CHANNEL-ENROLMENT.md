...
| a read-only claim that was never tested | `scripts/laneR/verify_seal.py` asserts the EFFECT, not the exception: the transport is replaced by a spy, 7 write routes must be refused **before reaching it**, 6 read routes and a GET must reach it, and no refused write may be counted. A guard that raised after handing the request over would pass a refusal test and still have written |
| a verifier that cannot fail | the guard was deliberately widened to allow `StopLeadInCampaign` and the verifier was re-run: it failed, named that route, and named both the allowance and the transport call. Then it was restored and `git diff` confirmed byte-identical |
| a redaction filter that agrees with itself | the first one flagged 258 hits, all of them name tokens inside ordinary words (`re<solve>s`, `<strang>er`, `<measured>`). Punctuated values - addresses, domains, slugs, credentials, `imp_` ids - are still matched as raw substrings; bare tokens are matched on word boundaries; and the 57 real names that are also ordinary English words are **listed** rather than counted clean. It caught two genuine leaks in this lane's own docstrings: a live `imp_` member id and the test identity's numeric one |
